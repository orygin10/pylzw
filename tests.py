...
class TestCompressionLZW(unittest.TestCase):
	...
	def test_file_compress(self):
		app.file_compress('tests/bible.txt', 'tests/bible.lzw')

		initial_size = os.stat('tests/bible.txt').st_size
		compressed_size = os.stat('tests/bible.lzw').st_size

		self.assertLess(compressed_size, initial_size)

	def test_file_decompress(self):
		with open('tests/bible.txt') as f:
			initial_bible = f.read()

		app.file_decompress('tests/bible.lzw', 'tests/t_bible.txt')
		with open('tests/t_bible.txt') as f:
			decompressed_bible = f.read()
		os.remove('tests/t_bible.txt')

		self.assertEqual(initial_bible, decompressed_bible)
...
if __name__ == '__main__':
	missing = []
	for file in ('b.bin', 'bible.lzw', 'bible.txt'):
